 ...
 */
package com.guzt.flowable.modeler.service;
...
public class PersistentTokenServiceImpl implements PersistentTokenService {
    ...
    @Resource
    private FlowableCommonAppProperties flowableCommonAppProperties;
    ...
    protected void initTokenCache() {
        FlowableCommonAppProperties.Cache cacheLoginUsers = flowableCommonAppProperties.getCacheLoginUsers();
        ...
        tokenCache = CacheBuilder.newBuilder().maximumSize(maxSize).expireAfterWrite(maxAge, TimeUnit.SECONDS).recordStats()
                .build(new CacheLoader<String, Token>() {
                    @Override
                    public Token load(final String tokenId) {
